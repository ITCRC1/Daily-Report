...
const bTh = "px-3 py-2.5 text-left font-medium text-white/60 whitespace-nowrap";
const bThN = "px-3 py-2.5 text-right font-medium text-white/60 whitespace-nowrap";
const bTd = "px-3 py-2 text-white/80 whitespace-nowrap";
const bTdN = "px-3 py-2 text-right tabular-nums text-white/80";
  ...
  return (
    <div className="space-y-3">
      <p className="text-[11px] text-white/40">
        Control diario de comps / in-house por tipo de habitación. El <span className="text-amber-300">saldo de arranque</span> es
      ...
      <div className="flex items-center gap-1">
        {(["rn", "pax"] as const).map((m) => (
          <button key={m} onClick={() => setMetric(m)}
            className={`rounded px-2.5 py-1 text-[11px] ${metric === m ? "bg-accent text-white" : "bg-[#1E2130] text-white/60 hover:text-white"}`}>
      ...
      {data && (
        <div className="overflow-x-auto rounded-xl border border-white/10 shadow-lg">
          <table className="w-full text-sm">
            ...
            <tbody>
              <tr className="border-t border-white/10 bg-[#16233a] font-bold text-emerald-300/90">
              ...
              {data.daily.map((d) => (
                <tr key={d.date} className="border-t border-white/5">
                  ...
                  {data.categories.map((c) => {
                    ...
                    return <td key={c} className={bTdN}>{val ? n(val) : <span className="text-white/20">—</span>}</td>;
                  })}
