...
export default function CashFlowForecast({ year }: { year: number }) {
  ...
  const neg = (v: number) => (v < 0 ? "text-rose-600" : "");
  const editCell = (key: string, val: string, onCh: (v: string) => void) => (
    <input type="text" inputMode="decimal" className={`${inp} ${numOf(val) < 0 ? "!text-rose-600" : ""}`}
  ...
  const beginCell = (scn: string, i: number, eff: number) => {
    ...
    return (
      <input type="text" inputMode="decimal" className={`${inp} ${shown < 0 ? "!text-rose-600" : ""} ${ov === "" ? "text-ink/60" : ""}`}
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h2 className="text-sm font-semibold text-ink/90">Full Year Cash Flow Forecast {year} vs Budget {year}</h2>
      ...
      <div className="overflow-x-auto rounded-lg border border-ink/10">
        <table className="text-[11px]">
          <thead className="bg-[#fcfcfb]">
            <tr>
              <th className="sticky left-0 z-10 bg-[#fcfcfb] px-2 py-1.5 text-left font-medium text-ink/70">&nbsp;</th>
              {cols.map((c) => <th key={c} className="px-1.5 py-1.5 text-right font-medium text-ink/70 whitespace-nowrap">{c}</th>)}
            </tr>
          </thead>
          {meta.order.map(({ key, label }) => {
              const c = calc(key);
              return (
                <tbody key={key} className="border-t-2 border-ink/15">
                  <tr className="bg-[#e8f0fb]"><td className="sticky left-0 z-10 bg-[#e8f0fb] px-2 py-1 font-bold text-sky-700" colSpan={cols.length + 1}>{label}</td></tr>
                  {/* Net Change in Cash — editable */}
                  <tr className="border-t border-ink/8">
                    <td className="sticky left-0 z-10 bg-[#f9f9f7] px-2 py-1 text-left text-ink/85 whitespace-nowrap">Net Change in Cash</td>
                    <td className={cellN} />
                  ...
                  <tr className="border-t border-ink/8">
                    <td className="sticky left-0 z-10 bg-[#f9f9f7] px-2 py-1 text-left text-ink/75 whitespace-nowrap">Beginning Cash</td>
                    <td className={cellN} />
                  ...
                  <tr className="border-t border-ink/8 bg-[#eeede7]">
                    <td className="sticky left-0 z-10 bg-[#eeede7] px-2 py-1 text-left font-medium text-ink/85 whitespace-nowrap">Ending Cash</td>
            ...
            {/* Variancias */}
            {variances.map((vr) => (
              <tbody key={vr.label} className="border-t-2 border-ink/15">
                <tr className="bg-[#fbf3e6]">
                  <td className="sticky left-0 z-10 bg-[#fbf3e6] px-2 py-1 font-medium text-amber-700/80 whitespace-nowrap">{vr.label}</td>
                  <td className={cellN} />
