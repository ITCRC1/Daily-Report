      ...
      {[1, 2, 3, 4].map((q) => {
        ...
        return (
          <button key={q} onClick={() => toggle(q)}
            className={`rounded px-2 py-0.5 font-mono ${c ? "bg-accent/30 text-white" : "bg-[#1E2130] text-white/60 hover:text-white"}`}
            title={c ? `Expand Q${q}` : `Collapse Q${q} into its total`}>
            {c ? "＋" : "－"} Q{q}
          </button>
        );
      })}
      <span className="text-white/30">— collapse (＋) to roll a quarter into one column</span>
    </div>
  );
}
  ...
  return (
    <section className="w-[calc(100vw-1.5rem)] -translate-x-1/2 relative left-1/2 space-y-4">
      ...
      <div className="print:hidden">
        <h1 className="text-xl font-semibold text-white">Tab 8 · On the Books</h1>
        <p className="text-xs text-white/50">
          Budget vs On-The-Books pacing ({year}, COWLCR){data?.snapshot_date && (
            <> · OTB cut <b className="text-white/70">{data.snapshot_date}</b></>
          )}. Replaces the manual Excel + PDF report.
          <br />
          <span className="text-amber-300/80">Nota: ADR/Occupancy del OTB son <b>gross of comps</b> — el forecast de Opera (history_forecast) no trae market_code para netear cortesías/in-house. No comparar 1:1 con el ADR neto de Tabs 6.6 / 7.3.</span>
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 rounded-lg border border-white/10 bg-[#161923] px-3 py-2 text-xs">
        <span className="font-medium text-white/70">OTB cut:</span>
        <label className="flex items-center gap-1 text-white/60">from
          <input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)}
            className="rounded border border-white/15 bg-[#0F1118] px-2 py-1 text-white" />
        </label>
        <label className="flex items-center gap-1 text-white/60">to
          <input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)}
            className="rounded border border-white/15 bg-[#0F1118] px-2 py-1 text-white" />
        </label>
        {data && (
          <span className="text-white/50">
            Final cut <b className="text-white/80">{data.snapshot_date ?? "—"}</b>
            {data.compare_snapshot_date
              ? <> vs initial <b className="text-white/80">{data.compare_snapshot_date}</b>
                  {moved != null && <> · OTB moved <b className={moved < 0 ? "text-red-400" : "text-emerald-400"}>{moved >= 0 ? "+" : ""}${money(moved)}</b></>}</>
              : <span className="text-white/30"> · no earlier cut yet (only one snapshot ingested so far)</span>}
          </span>
        )}
...
function ONTBReport({ data }: { data: Report }) {
  ...
  return (
    <div className="space-y-1">
      ...
      <div className="overflow-auto rounded-lg border border-white/10 max-h-[72vh]">
        <table className="w-full text-xs">
          <thead className="bg-[#1E2130]">
            <tr>
              <th className={`${thl} sticky left-0 top-0 z-30 bg-[#1E2130]`}>Metric</th>
              {cols.map((c, i) => (
                <th key={i} className={`${th} sticky top-0 z-20 ${c.kind === "q" ? `${qbg} text-white/80` : "bg-[#1E2130]"}`}>{c.label}</th>
              ))}
              <th className={`${th} sticky top-0 z-20 bg-[#1E2130] border-l border-white/10`}>Total</th>
            </tr>
          </thead>
          <tbody>
            ...
            {groupHead("Diff (OTB − Budget)", "text-amber-300")}
            ...
            <tr className="border-t-2 border-white/20 bg-emerald-900/20 font-bold">
              <td className="sticky left-0 z-10 bg-[#0f1a15] px-3 py-2 text-left text-emerald-200">NET GAP</td>
              {cols.map((c, i) => {
              ...
              <td className={`px-2 py-2 text-right border-l border-white/10 ${neg(data.net_gap_total)}`}>${money(data.net_gap_total)}</td>
            </tr>
            ...
            {hasWoW ? (
              <tr className="border-t border-white/5 font-medium">
                ...
                <td className={`px-2 py-1 text-right border-l border-white/10 ${move! < 0 ? "text-red-400" : move! > 0 ? "text-emerald-400" : "text-white/40"}`}>
                  {move! > 0 ? "+" : ""}${money(move!)}
                </td>
              </tr>
            ) : (
      ...
      {hasWoW && (
        <div className={`rounded-lg border px-3 py-2 text-sm font-semibold ${move! > 0 ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-300" : move! < 0 ? "border-red-500/30 bg-red-500/10 text-red-300" : "border-white/15 bg-white/5 text-white/70"}`}>
        ...
        <table className="w-full text-xs">
          <thead className="bg-[#1E2130]">
            <tr>
              ...
              <th className={`${th} border-l border-white/10`}>Total</th>
            </tr>
          </thead>
          <tbody>
            {groupHead("Total Revenue")}
            {Row("Revenue Budget", "text-sky-300", budget, sumB)}
            {Row("OTB Revenue (OPERA file)", "text-white/45", otbRev, sumOTB)}
            {Row("＋ On-Property (12.6% est.)", "text-white/45", onProp, sumOnProp)}
...
function Heatmap({ year, asOf, refreshKey }: { year: number; asOf: string; refreshKey: number }) {
  ...
  if (err) return <div className="text-xs text-amber-200">No daily OTB data yet. ({err})</div>;
  if (!d) return <div className="text-xs text-white/50">Loading…</div>;
  if (!d.snapshot_date) return <div className="text-xs text-amber-200">No OTB snapshot yet — ingest a day with history_forecast files.</div>;

  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-3 text-[11px] text-white/60">
        <span>OTB daily occupancy — snapshot <b className="text-white/80">{d.snapshot_date}</b>. Occ% = Rooms Sold / Inventory.</span>
        <span className="flex items-center gap-2">
          {["OK", "MID", "HIGH", "CRITICAL"].map((k) => (
            <span key={k} className={`rounded px-1.5 py-0.5 ${RISK_BG[k]}`}>{k}</span>
          ))}
        </span>
        <span className="text-white/40">Action: OK→HOLD · MID→WATCH · HIGH→RATE · CRITICAL→PUSH</span>
      </div>
      <div className="print-heatmap-wrap overflow-x-auto rounded-lg border border-white/10">
        <table className="print-heatmap text-[10px]">
          <thead className="bg-[#1E2130]">
            <tr>
              <th rowSpan={2} className="sticky left-0 bg-[#1E2130] px-2 py-1 text-left text-white/60">Month</th>
              ...
              <th rowSpan={2} className="border-l border-white/15 px-2 py-1 text-right align-bottom text-white/60">Occ&nbsp;Var</th>
            </tr>
            <tr>
              <th className="border-l border-white/15 px-2 py-0.5 text-right text-emerald-300/70">Noches</th>
              ...
              <th className="border-l border-white/15 px-2 py-0.5 text-right text-sky-300/70">Noches</th>
          ...
          <tbody>
            {d.months.map((mo) => (
              <tr key={mo.month} className="border-t border-white/5">
                <td className="sticky left-0 bg-[#0F1118] px-2 py-1 text-left text-white/70 whitespace-nowrap">{mo.name}</td>
                ...
                <td className="border-l border-white/15 px-2 py-1 text-right text-white/80">{dec(mo.noches)}</td>
                <td className="px-2 py-1 text-right text-white/80">{dec(mo.pax)}</td>
                <td className="px-2 py-1 text-right text-white/80">${money(mo.adr)}</td>
                <td className="px-2 py-1 text-right font-medium text-emerald-300">{pct(mo.otb_occ)}</td>
                <td className="border-l border-white/15 px-2 py-1 text-right text-white/60">{dec(mo.budget_noches)}</td>
                <td className="px-2 py-1 text-right text-white/60">{dec(mo.budget_pax)}</td>
                <td className="px-2 py-1 text-right text-white/60">${money(mo.budget_adr)}</td>
                <td className="px-2 py-1 text-right font-medium text-sky-300">{pct(mo.budget_occ)}</td>
                <td className={`border-l border-white/15 px-2 py-1 text-right font-medium ${mo.variance < 0 ? "text-red-400" : "text-emerald-400"}`}>
      ...
      <p className="text-[11px] text-white/40">Numbers = daily occupancy %. Right columns = monthly Noches / Pax / ADR / Occ% for <span className="text-emerald-300">Forecast (OTB)</span> vs <span className="text-sky-300">Budget</span>, plus occupancy variance (percentage points). Hover a cell for rooms sold/available and the recommended action.</p>
    </div>
  );
}
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-white/60">
        ...
        {d.count < 2 && <span className="text-amber-300"> Accumulating: {d.count} snapshot so far. The trend fills in as more days are ingested each week.</span>}
      </p>
      <div className="space-y-1.5">
        {d.snapshots.map((s) => (
          <div key={s.snapshot_date} className="flex items-center gap-3">
            <span className="w-24 text-right text-xs text-white/70">{s.snapshot_date}</span>
            <div className="relative h-6 flex-1 rounded bg-[#161923]">
              ...
              <span className="absolute inset-y-0 left-2 flex items-center text-[11px] text-white">${money(s.otb_total_revenue)}</span>
            </div>
            <span className={`w-28 text-right text-xs ${s.delta === null ? "text-white/30" : neg(s.delta)}`}>
              {s.delta === null ? "—" : `${s.delta >= 0 ? "+" : ""}$${money(s.delta)}`}
            </span>
          </div>
        ))}
      </div>
      <p className="text-[11px] text-white/40">Right column = change vs the previous snapshot (week-over-week booking pace).</p>
    </div>
  );
}
  ...
  return (
    <div className="space-y-2 max-w-[1500px] mx-auto">
      <div className="rounded-lg border border-white/10 bg-[#0F1118] p-3">
        <div className="mb-1 text-center text-sm font-semibold text-white/90">OTB Revenue trending versus Budget {data.year}</div>
        <div className="overflow-x-auto">
          <svg viewBox={`0 0 ${W} ${H}`} className="w-full min-w-[760px]" role="img">
            {/* gridlines + y labels */}
            {gridY.map((g) => (
              <g key={g}>
                <line x1={L} y1={y(g)} x2={W - R} y2={y(g)} stroke="#ffffff14" />
                <text x={L - 8} y={y(g) + 3} textAnchor="end" fontSize="10" fill="#ffffff66">${(g / 1000).toLocaleString()}k</text>
              </g>
            ))}
            {/* bars */}
            {m.map((d, i) => {
              ...
              return (
                <g key={i}>
                  <rect x={bx} y={y(budget[i])} width={barW} height={T + plotH - y(budget[i])} fill="#4472C4">
                  ...
                  <text x={bx + barW / 2} y={y(budget[i]) - 4} textAnchor="middle" fontSize="9" fill="#ffffffcc">{kfmt(budget[i])}</text>
                  {/* forecast value label */}
                  <text x={fx + barW / 2} y={y(forecast[i]) - 4} textAnchor="middle" fontSize="9" fill="#ffffffcc">{kfmt(forecast[i])}</text>
                  ...
                  {/* month label */}
                  <text x={cx(i)} y={T + plotH + 15} textAnchor="middle" fontSize="11" fill="#ffffff99">{MONTHS[i]}</text>
                </g>
              );
            })}
        ...
        <div className="mt-1 flex flex-wrap items-center justify-center gap-4 text-[11px] text-white/70">
          <span className="flex items-center gap-1"><span className="inline-block h-3 w-3 rounded-sm" style={{ background: "#4472C4" }} /> Revenue Budget</span>
          ...
          <span className="flex items-center gap-1"><span className="inline-block h-0.5 w-5" style={{ background: "#ff0000" }} /> 2-per. Moving Avg (Forecast)</span>
        </div>
      </div>
      <p className="text-[11px] text-white/40">
...
function Analisis2027({ asOf, refreshKey }: { asOf: string; refreshKey: number }) {
  ...
  if (err) return <div className="text-xs text-amber-200">({err})</div>;
  if (!y27 || !y26) return <div className="text-xs text-white/50">Loading…</div>;
  ...
  const seriesMeta = [
    { label: "Budget 2027", color: "#4472C4" },
    { label: "OTB 2026", color: "#8C9196" },
    { label: "OTB 2027", color: "#ED7D31" },
  ];
  ...
  return (
    <div className="space-y-2">
      <div className="rounded-lg border border-white/10 bg-[#0F1118] p-3">
        <div className="mb-1 text-center text-sm font-semibold text-white/90">Análisis 2027 — Budget 2027 · OTB 2026 · OTB 2027 · {metricLbl} (snapshot {y27.snapshot_date ?? asOf})</div>
        {!has27 && (
          ...
          <div className="flex items-center gap-1">
            {(["total", "rooms"] as const).map((mk) => (
              <button key={mk} onClick={() => setMetric(mk)}
                className={`rounded px-2.5 py-1 text-[11px] ${metric === mk ? "bg-accent text-white" : "bg-[#1E2130] text-white/60 hover:text-white"}`}>
        ...
        <div className="overflow-x-auto">
          <svg viewBox={`0 0 ${W} ${H}`} className="w-full min-w-[760px]" role="img">
            ...
            {fyTicks.map((t, i) => (
              <text key={i} x={W - R + 6} y={fyY(t) + 3} textAnchor="start" fontSize="9" fill="#ffffff66">{mfmt(t)}</text>
            ))}
                  ...
                  {seriesMeta.map((s, si) => {
                    ...
                    return (
                      <g key={si}>
                        ...
                        {v > 0 && (
                          <text x={lx} y={topY - 5} textAnchor="middle" fontSize="9" fontWeight="600" fill="#ffffffee">{kfmt(v)}</text>
                        )}
                      </g>
                    );
                  })}
                  ...
                  {seriesMeta.map((s, si) => {
                    ...
                    return (
                      <g key={si}>
                        ...
                        {v > 0 && (
                          <text x={lx} y={topY - 5} textAnchor="middle" fontSize="9" fontWeight="700" fill="#ffffffee">{mfmt(v)}</text>
                        )}
                      </g>
                    );
                  })}
                  <text x={fyCx} y={y0 + 15} textAnchor="middle" fontSize="11" fontWeight="700" fill="#ffffffcc">Full Year</text>
                </g>
              );
            })()}
          </svg>
      ...
      <div className="flex flex-wrap gap-3 text-[11px]">
        <div className="rounded-lg border border-white/10 bg-[#161923] px-3 py-2"><span className="text-white/50">Budget 2027:</span> <b className="text-sky-300">${money(totalB27)}</b></div>
        <div className="rounded-lg border border-white/10 bg-[#161923] px-3 py-2"><span className="text-white/50">OTB 2026:</span> <b className="text-white/70">${money(total26)}</b></div>
        <div className="rounded-lg border border-white/10 bg-[#161923] px-3 py-2"><span className="text-white/50">OTB 2027:</span> <b className="text-orange-300">${money(total27)}</b></div>
        <div className="rounded-lg border border-white/10 bg-[#161923] px-3 py-2"><span className="text-white/50">OTB 2027 vs Budget:</span> <b className={gapBudget >= 0 ? "text-emerald-300" : "text-red-300"}>{gapBudget >= 0 ? "+" : "−"}${money(Math.abs(gapBudget))}</b></div>
      </div>
      <p className="text-[11px] text-white/40">
        {metricLbl} por {cols.length === 12 ? "mes" : "período"} (snapshot {y27.snapshot_date ?? asOf}): <span className="text-sky-300">Budget 2027</span> · <span className="text-white/70">OTB 2026</span> · <span className="text-orange-300">OTB 2027</span>. Toggle <b>Full/Rooms Revenue</b> y colapsá cuartos (＋Q) para ver el total del trimestre. Las líneas gruesas separan los cuartos; la columna <b>Full Year</b> (tras la línea gruesa) es el total anual y usa su <b>propia escala</b> (eje derecho), para no aplastar los meses. "OTB 2027 vs Budget" = cuánto falta reservar para llegar al presupuesto.
      </p>
    </div>
  );
}
...
function OccupancyTrend({ data }: { data: Report }) {
  ...
  return (
    <div className="space-y-2 max-w-[1500px] mx-auto">
      <div className="rounded-lg border border-white/10 bg-[#0F1118] p-3">
        <div className="mb-1 text-center text-sm font-semibold text-white/90">OTB Occupancy trending versus Budget {data.year}</div>
        <div className="overflow-x-auto">
          <svg viewBox={`0 0 ${W} ${H}`} className="w-full min-w-[760px]" role="img">
            {grid.map((g) => (
              <g key={g}>
                <line x1={L} y1={y(g)} x2={W - R} y2={y(g)} stroke="#ffffff12" />
                <text x={L - 8} y={y(g) + 3} textAnchor="end" fontSize="10" fill="#ffffff66">{Math.round(g * 100)}%</text>
              </g>
            ))}
            {/* budget line */}
            <polyline points={poly(budget)} fill="none" stroke="#4472C4" strokeWidth="2.5" />
            {/* forecast line */}
            <polyline points={poly(forecast)} fill="none" stroke="#ED7D31" strokeWidth="2.5" />
            {m.map((d, i) => (
              <g key={i}>
                <circle cx={cx(i)} cy={y(budget[i])} r="3" fill="#4472C4" />
                ...
                <text x={cx(i)} y={T + plotH + 16} textAnchor="middle" fontSize="11" fill="#ffffff99">{MONTHS[i]}</text>
              </g>
            ))}
          </svg>
        </div>
        <div className="mt-1 flex flex-wrap items-center justify-center gap-4 text-[11px] text-white/70">
          <span className="flex items-center gap-1"><span className="inline-block h-0.5 w-5" style={{ background: "#4472C4" }} /> % Budget {data.year}</span>
      ...
      <p className="text-[11px] text-white/40">Monthly occupancy: Budget vs On-The-Books forecast (snapshot {data.snapshot_date}).</p>
    </div>
  );
}
...
function VariancePie({ data }: { data: Report }) {
  ...
  if (!slices.length) return <div className="text-xs text-emerald-300">No shortfall — On-The-Books meets or beats budget in every month. 🎉</div>;

  return (
    <div className="space-y-2">
      <div className="rounded-lg border border-white/10 bg-[#0F1118] p-3">
        <div className="mb-2 text-center text-sm font-semibold text-white/90">Variance breakdown — where the year&apos;s gap comes from</div>
              ...
              <tbody>
                {paths.map((p, i) => (
                  <tr key={i} className="border-t border-white/5">
                    ...
                    <td className="px-2 py-1 text-right text-red-300">${money(p.s.val)}</td>
                    <td className="px-2 py-1 text-right text-white/70">{Math.round(p.frac * 100)}%</td>
                  </tr>
                ))}
                <tr className="border-t border-white/20 font-bold">
                  <td className="px-2 py-1 text-left">Total gap</td>
                  <td className="px-2 py-1 text-right text-red-400">${money(total)}</td>
