...
const th = "px-2 py-1.5 text-right text-[11px] font-medium text-ink/60 whitespace-nowrap";
const tdN = "px-2 py-1 text-right tabular-nums whitespace-nowrap";
const negC = (v: number | null) => (v !== null && v < 0 ? "text-rose-600" : "text-ink/75");
...
function Row({ label, col, t, bold }: { label: string; col: Col | null; t: FT; bold?: boolean }) {
  ...
  const rc = bold ? "bg-[#eeede7] font-medium" : "";
  const c = bold ? "text-ink/90" : "text-ink/75";
  return (
    <tr className={`border-t border-ink/8 ${rc}`}>
...
const GH = (label: string) => (
  <tr className="bg-[#f2f1ec]"><td className="px-3 py-1.5 text-left text-[11px] font-bold uppercase tracking-wide text-sky-700" colSpan={12}>{label}</td></tr>
);
      ...
      <div className="overflow-x-auto rounded-lg border border-ink/10">
        <table className="text-xs">
          <thead className="bg-[#fcfcfb]">
            <tr>
              <th className="px-3 py-1.5 text-left text-[11px] font-medium text-ink/60">Summary</th>
              <th className={th}>Today</th>
              <th className={`${th} border-l border-ink/10`}>M-T-D Actual</th>
              <th className={th}>Forecast</th><th className={th}>Var</th>
              <th className={`${th} border-l border-ink/10`}>Budget</th><th className={th}>Var</th>
              <th className={`${th} border-l border-ink/10`}>Año ant.</th><th className={th}>Var</th>
              <th className={`${th} border-l border-ink/10`}>Budget Mes</th>
...
const segTh = "px-2 py-1.5 text-right text-[11px] font-medium text-ink/60 whitespace-nowrap";
function SegCells({ m, bold }: { m: SegMetrics; bold?: boolean }) {
  const c = bold ? "text-ink/90" : "text-ink/75";
  return (
    <>
      <td className={`${tdN} border-l border-ink/10 ${c}`}>{fmt(m.rn, "int")}</td>
...
function RoomsBySegment({ date }: { date: string }) {
  ...
  return (
    <div className="space-y-2">
      ...
      {err && <div className="rounded border border-red-500/30 bg-red-500/5 px-2 py-1.5 text-xs text-red-600">{err}</div>}
      {empty && <div className="rounded border border-ink/10 bg-[#fcfcfb] px-2 py-1.5 text-xs text-ink/60">Sin datos de segmento para esta fecha.</div>}
      {d && !empty && (
        <div className="overflow-x-auto rounded-lg border border-ink/10">
          <table className="text-xs">
            <thead className="bg-[#fcfcfb]">
              <tr>
                <th className="px-3 py-1.5 text-left text-[11px] font-medium text-ink/60" rowSpan={2}>Market Segment</th>
                <th className="px-2 py-1 text-center text-[11px] font-semibold text-sky-700 border-l border-ink/10" colSpan={4}>Today</th>
                <th className="px-2 py-1 text-center text-[11px] font-semibold text-sky-700 border-l border-ink/10" colSpan={4}>M-T-D</th>
              </tr>
              <tr>
                <th className={`${segTh} border-l border-ink/10`}>RN</th><th className={segTh}>Occ%</th><th className={segTh}>Revenue</th><th className={segTh}>ADR</th>
                <th className={`${segTh} border-l border-ink/10`}>RN</th><th className={segTh}>Occ%</th><th className={segTh}>Revenue</th><th className={segTh}>ADR</th>
              </tr>
            </thead>
            <tbody>
              {d.groups.map((g) => (
                <Fragment key={g.group}>
                  {GH(g.group)}
                  {g.segments.map((s) => (
                    <tr key={s.market_code} className="border-t border-ink/8">
                      <td className="px-3 py-1 text-left text-ink/85 whitespace-nowrap">
                        <span className="text-ink/60">{s.market_code}</span>{s.description ? ` · ${s.description}` : ""}
                      </td>
                      <SegCells m={s.today} />
                      <SegCells m={s.mtd} />
                    </tr>
                  ...
                  <tr className="border-t border-ink/10 bg-[#eeede7] font-medium">
                    <td className="px-3 py-1 text-left text-ink/90 whitespace-nowrap">Subtotal {g.group}</td>
                    <SegCells m={g.subtotal.today} bold />
                    <SegCells m={g.subtotal.mtd} bold />
                  </tr>
                </Fragment>
              ))}
              <tr className="border-t-2 border-ink/15 bg-[#f2f1ec] font-semibold">
                <td className="px-3 py-1.5 text-left text-ink whitespace-nowrap">TOTAL</td>
...
function RDRow({ label, today, mtd, bud, bold, indent }: {
  ...
  const c = bold ? "text-ink/90" : "text-ink/75";
  return (
    <tr className={`border-t border-ink/8 ${bold ? "bg-[#eeede7] font-medium" : ""}`}>
      <td className={`px-3 py-1 text-left whitespace-nowrap ${bold ? "text-ink/90" : "text-ink/85"} ${indent ? "pl-6" : ""}`}>{label}</td>
      <td className={`${tdN} ${c}`}>{fmt(today, "usd")}</td>
      <td className={`${tdN} border-l border-ink/10 ${negC(mtd)}`}>{fmt(mtd, "usd")}</td>
      <td className={`${tdN} text-ink/60`}>{fmt(bud, "usd")}</td>
      ...
      {d && (
        <div className="overflow-x-auto rounded-lg border border-ink/10">
          <table className="text-xs">
            <thead className="bg-[#fcfcfb]">
              <tr>
                <th className="px-3 py-1.5 text-left text-[11px] font-medium text-ink/60">Revenue Detail</th>
                <th className={segTh}>Today</th>
                <th className={`${segTh} border-l border-ink/10`}>M-T-D Actual</th>
            ...
            <tbody>
              ...
              <tr className="border-t-2 border-ink/15 bg-[#f2f1ec] font-semibold">
                <td className="px-3 py-1.5 text-left text-ink whitespace-nowrap">TOTAL REVENUE</td>
                <td className={`${tdN} text-ink/90`}>{fmt(d.total.today, "usd")}</td>
                <td className={`${tdN} border-l border-ink/10 ${negC(d.total.mtd_actual)}`}>{fmt(d.total.mtd_actual, "usd")}</td>
                <td className={`${tdN} text-ink/60`}>{fmt(d.total.mtd_budget, "usd")}</td>
...
function FBCells({ m, bold }: { m: FBMetrics; bold?: boolean }) {
  ...
  return (
    <>
      <td className={`${tdN} border-l border-ink/10 ${c}`}>{fmt(m.food, "usd")}</td>
      <td className={`${tdN} ${c}`}>{fmt(m.beverage, "usd")}</td>
      <td className={`${tdN} ${c}`}>{fmt(m.misc, "usd")}</td>
      <td className={`${tdN} ${bold ? "text-ink/90" : "text-ink/85"} font-medium`}>{fmt(m.total, "usd")}</td>
    </>
  );
}
...
function FBCovAvg({ covers, avg, bold, edit }: {
  ...
  return (
    <>
      <td className={`${tdN} border-l border-ink/10 ${c}`}>
        {edit ? (
          <input
            value={edit.value}
            disabled={edit.busy}
            ...
            className="w-16 rounded border border-ink/12 bg-[#f9f9f7] px-1 py-0.5 text-right text-xs text-ink tabular-nums focus:border-accent focus:outline-none disabled:opacity-40"
          />
        ) : (
          fmt(covers, "int")
        )}
      </td>
      <td className={`${tdN} ${bold ? "text-ink/90 font-medium" : "text-ink/75"}`}>
        {covers ? fmt(avg, "usd") : <span className="text-ink/45">—</span>}
      </td>
    </>
  );
}
    ...
    <div className="space-y-2">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <button
          ...
          className="rounded bg-ink/5 px-2.5 py-1 text-[11px] text-ink/75 hover:bg-ink/8"
        >
        ...
        {upMsg && <span className={upMsg.startsWith("✓") ? "text-[11px] text-emerald-600" : "text-[11px] text-ink/60"}>{upMsg}</span>}
      </div>
      {err && <div className="rounded border border-red-500/30 bg-red-500/5 px-2 py-1.5 text-xs text-red-600">{err}</div>}
      {empty && <div className="rounded border border-ink/10 bg-[#fcfcfb] px-2 py-1.5 text-xs text-ink/60">Sin detalle de F&B en Integrity para esta fecha.</div>}
      {d && !empty && (
        <div className="overflow-x-auto rounded-lg border border-ink/10">
          <table className="text-xs">
            <thead className="bg-[#fcfcfb]">
              <tr>
                <th className="px-3 py-1.5 text-left text-[11px] font-medium text-ink/60" rowSpan={2}>Sub-departamento / Meal Period</th>
                <th className="px-2 py-1 text-center text-[11px] font-semibold text-sky-700 border-l border-ink/10" colSpan={6}>Today</th>
                <th className="px-2 py-1 text-center text-[11px] font-semibold text-sky-700 border-l border-ink/10" colSpan={6}>M-T-D</th>
              </tr>
              <tr>
                <th className={`${segTh} border-l border-ink/10`}>Food</th><th className={segTh}>Beverage</th><th className={segTh}>Misc</th><th className={segTh}>Total</th>
                <th className={`${segTh} border-l border-ink/10`}>Covers</th><th className={segTh}>Avg Check</th>
                <th className={`${segTh} border-l border-ink/10`}>Food</th><th className={segTh}>Beverage</th><th className={segTh}>Misc</th><th className={segTh}>Total</th>
                <th className={`${segTh} border-l border-ink/10`}>Covers</th><th className={segTh}>Avg Check</th>
              </tr>
            </thead>
            <tbody>
                    ...
                    {o.meals.map((ml) => {
                      ...
                      return (
                        <tr key={k} className="border-t border-ink/8">
                          <td className="px-3 py-1 pl-8 text-left text-ink/65 whitespace-nowrap">{ml.meal_period}</td>
                          <FBCells m={ml.today} />
                          <FBCovAvg
              ...
              <tr className="border-t-2 border-ink/15 bg-[#f2f1ec] font-semibold">
                <td className="px-3 py-1.5 text-left text-ink whitespace-nowrap">TOTAL F&B</td>
      ...
      {d && !empty && (
        <div className="flex flex-wrap gap-3 pt-1">
          ...
          <div className="min-w-[220px] flex-1 rounded-lg border border-ink/10 bg-[#fcfcfb] p-3">
            ...
            <div className="mt-2 flex items-end justify-between gap-3">
              <div>
                <div className="text-[10px] text-ink/50">Today</div>
                <input type="number" min={0} value={cust} disabled={saving}
                  ...
                  className="mt-0.5 w-24 rounded border border-ink/12 bg-[#f9f9f7] px-2 py-1 text-right tabular-nums text-ink/90 focus:border-accent focus:outline-none" />
              </div>
              <div className="text-right"><div className="text-[10px] text-ink/50">M-T-D</div><div className="text-base font-semibold text-ink/90 tabular-nums">{fmt(d.customers.mtd, "int")}</div></div>
            </div>
          </div>
          {/* Average Check */}
          <div className="min-w-[220px] flex-1 rounded-lg border border-ink/10 bg-[#eeede7] p-3">
            <div className="text-[11px] uppercase tracking-wide text-ink/60">Average Check</div>
            <div className="mt-2 flex items-end justify-between gap-3">
              <div><div className="text-[10px] text-ink/50">Today</div><div className="text-base font-semibold text-ink tabular-nums">{fmt(d.avg_check.today, "usd")}</div></div>
              <div className="text-right"><div className="text-[10px] text-ink/50">M-T-D</div><div className="text-base font-semibold text-ink tabular-nums">{fmt(d.avg_check.mtd, "usd")}</div></div>
            </div>
          </div>
        </div>
      )}
      ...
      {d && (
        <div className="overflow-x-auto rounded-lg border border-ink/10">
          <table className="text-xs">
            <thead className="bg-[#fcfcfb]">
              <tr>
                <th className="px-3 py-1.5 text-left text-[11px] font-medium text-ink/60">F&B Revenue</th>
                <th className={th}>Today</th>
                <th className={`${th} border-l border-ink/10`}>M-T-D Actual</th>
                <th className={th}>Forecast</th><th className={th}>Var</th>
                <th className={`${th} border-l border-ink/10`}>Budget</th><th className={th}>Var</th>
...
function BeverageDetail({ date }: { date: string }) {
  ...
  return (
    <div className="space-y-2">
      ...
      {err && <div className="rounded border border-red-500/30 bg-red-500/5 px-2 py-1.5 text-xs text-red-600">{err}</div>}
      {empty && <div className="rounded border border-ink/10 bg-[#fcfcfb] px-2 py-1.5 text-xs text-ink/60">Sin detalle de Beverage en Integrity para esta fecha.</div>}
      {d && !empty && (
        <div className="overflow-x-auto rounded-lg border border-ink/10">
          <table className="text-xs">
            <thead className="bg-[#fcfcfb]">
              <tr>
                <th className="px-3 py-1.5 text-left text-[11px] font-medium text-ink/60">Beverage · Concepto</th>
                <th className={th}>Today</th>
                <th className={`${th} border-l border-ink/10`}>M-T-D Actual</th>
                <th className={th}>Forecast</th><th className={th}>Var</th>
                <th className={`${th} border-l border-ink/10`}>Budget</th><th className={th}>Var</th>
      ...
      {d && (
        <div className="overflow-x-auto rounded-lg border border-ink/10">
          <table className="text-xs w-full">
            <thead className="bg-[#fcfcfb]">
              <tr>
                <th className="px-3 py-1.5 text-left text-[11px] font-medium text-ink/60">Spa</th>
                <th className={th}>Today</th>
                <th className={`${th} border-l border-ink/10`}>M-T-D</th>
              </tr>
            </thead>
            <tbody>
              <tr className="border-t border-ink/8">
                <td className="px-3 py-1.5 text-left text-ink/85 whitespace-nowrap">Monto Spa</td>
                <td className={`${tdN} text-ink/75`}>{fmt(d.revenue.today, "usd")}</td>
                <td className={`${tdN} border-l border-ink/10 text-ink/75`}>{fmt(d.revenue.mtd, "usd")}</td>
              </tr>
              <tr className="border-t border-ink/8">
                <td className="px-3 py-1.5 text-left text-ink/85 whitespace-nowrap">Total Treatments</td>
                <td className={`${tdN}`}>
                  <input
                    type="number" min={0} value={input} disabled={saving}
                    onChange={(e) => setInput(e.target.value)}
                    onKeyDown={(e) => { if (e.key === "Enter") save(); }}
                    className="w-24 rounded border border-ink/12 bg-[#f9f9f7] px-2 py-1 text-right tabular-nums text-ink/90 focus:border-accent focus:outline-none"
                  />
                </td>
                <td className={`${tdN} border-l border-ink/10 text-ink/75`}>{fmt(d.treatments.mtd, "int")}</td>
              </tr>
              <tr className="border-t border-ink/10 bg-[#eeede7] font-medium">
                <td className="px-3 py-1.5 text-left text-ink/90 whitespace-nowrap">Average Rate</td>
                <td className={`${tdN} text-ink/90`}>{fmt(d.avg_rate.today, "usd")}</td>
                <td className={`${tdN} border-l border-ink/10 text-ink/90`}>{fmt(d.avg_rate.mtd, "usd")}</td>
              </tr>
            </tbody>
          </table>
        </div>
  ...
  if (printing) {
    return (
      <section className="print-report w-[calc(100vw-1.5rem)] -translate-x-1/2 relative left-1/2 space-y-4">
        <div className="no-print sticky top-0 z-20 flex flex-wrap items-center gap-2 rounded-lg border border-accent/40 bg-[#fcfcfb] px-3 py-2">
        ...
        {SUBTABS.map((s) => (
          <section key={s.id} className="print-section space-y-2">
            <h2 className="print-section-title text-sm font-semibold text-ink/90">{s.label}</h2>
