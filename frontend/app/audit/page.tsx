...
const ESTADO_STYLE: Record<string, string> = {
  OK: "text-emerald-400", DISCREPANCIA: "text-amber-400",
  "FALTA EN INTEGRITY": "text-red-400", "FALTA EN OPERA": "text-red-400",
  INTERNO: "text-white/40", DIF_OPERATIVA: "text-amber-400",
};
...
function Pending({ what, needs }: { what: string; needs: string }) {
  return (
    <div className="rounded-lg border border-dashed border-white/15 bg-[#1E2130]/50 p-6 text-sm text-white/60">
      <b className="text-white/80">{what}</b> — pending. {needs}
    </div>
  );
}
  ...
  return (
    <section className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-white">Tab 2 · Daily Audit</h1>
        ...
        <div className="flex items-center gap-2">
          {data && (
            <span className={`rounded px-2 py-1 text-[11px] font-medium ${
              data.status === "cerrado" ? "bg-emerald-500/15 text-emerald-400" : "bg-white/10 text-white/60"}`}>
          ...
          {data?.status === "cerrado" ? (
            <>
              <button onClick={refresh} disabled={loading}
                className="rounded bg-white/10 px-3 py-2 text-xs font-medium text-white hover:bg-white/20 disabled:opacity-50">
                {loading ? "…" : "🔄 REFRESH"}
              </button>
          ...
          <button onClick={applyAudit} disabled={loading}
            ...
            className="rounded bg-indigo-600 px-3 py-2 text-xs font-medium text-white hover:bg-indigo-500 disabled:opacity-50">
            {loading ? "…" : "🛡 Apply Audit"}
          </button>
      ...
      {data && !data.gate.allowed && data.status !== "cerrado" && (
        <div className="rounded border border-red-500/30 bg-red-500/5 px-3 py-2 text-xs text-red-300">
          ...
          {!showOverride ? (
            <button onClick={() => setShowOverride(true)} className="rounded bg-white/10 px-2 py-1 text-[11px] text-white/80 hover:bg-white/20">
            ...
            <div className="flex gap-2">
              <input value={overrideNote} onChange={(e) => setOverrideNote(e.target.value)}
                ...
                className="w-80 rounded border border-white/15 bg-[#0F1118] px-2 py-1 text-white" />
              ...
              <button onClick={() => setShowOverride(false)} className="rounded bg-white/10 px-2 py-1 text-[11px] text-white/70">✕</button>
            </div>
          )}
        </div>
      )}
      ...
      {msg && (
        <div className={`rounded border p-3 text-sm ${
          msg.startsWith("✓") ? "border-emerald-500/30 bg-emerald-500/5 text-emerald-300"
            : msg.startsWith("Error") || msg.startsWith("Could not confirm") ? "border-red-500/30 bg-red-500/5 text-red-300"
            : "border-white/10 bg-[#1E2130] text-white/70"
        }`}>
          {msg}
        </div>
      )}
      ...
      {data && (
        <>
          <div className="grid grid-cols-4 gap-3">
            <Kpi label="Reconciled" value={data.kpis.ok} tone="text-emerald-400" />
            <Kpi label="Discrepancies" value={data.kpis.discrepancia} tone={data.kpis.discrepancia ? "text-amber-400" : "text-white"} />
            <Kpi label="Missing" value={data.kpis.faltante} tone={data.kpis.faltante ? "text-red-400" : "text-white"} />
            <Kpi label="Internal" value={data.kpis.interno} tone="text-white/60" />
          </div>
          ...
          <div className="overflow-x-auto">
            ...
            {tab === "2.10" && (
              <div className="space-y-6">
                <HallazgosYTD />
                <div>
                  <h3 className="mb-2 text-sm font-medium text-white/70">Findings for the day ({day})</h3>
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-3 gap-3">
        <Kpi label="Revenue (Opera)" value={`$${money(d.revenue_total)}`} tone="text-emerald-400" />
        <Kpi label="Non-Revenue" value={`$${money(d.nonrev_total)}`} tone="text-white/80" />
        <Kpi label="Payments" value={`$${money(d.payment_total)}`} tone="text-sky-400" />
      </div>
      <table className="w-full rounded-lg border border-white/10 text-xs">
        ...
        <tbody>
          {d.category_summary.map((c) => {
            ...
            return (
              <tr key={c.categoria} className="border-t border-white/5">
                ...
                <td className={`${tdN} ${ok ? negColor(c.diferencia) : "text-amber-400"}`}>${money(c.diferencia)}</td>
                <td className={`px-3 py-1.5 font-medium ${ok ? "text-emerald-400" : "text-amber-400"}`}>{ok ? "✓ Reconciled" : "⚠ Review"}</td>
              </tr>
            );
          })}
        </tbody>
...
function Recon({ rows, empty }: { rows: Row[]; empty?: string }) {
  if (empty && rows.length === 0)
    return <div className="rounded-lg border border-white/10 bg-[#1E2130] p-6 text-sm text-emerald-400">{empty}</div>;
  return (
    <table className="w-full rounded-lg border border-white/10 text-xs">
      <thead className="bg-[#1E2130]"><tr>
      ...
      <tbody>
        {rows.map((r, i) => (
          <tr key={`${r.tcode}-${r.nombre}-${i}`} className="border-t border-white/5">
            <td className="px-3 py-1.5 font-mono text-white/80">{r.tcode}</td>
            <td className={td}>{r.description}</td>
            <td className="px-3 py-1.5 text-white/50">{r.type}</td>
            <td className={`${tdN} ${negColor(r.opera)}`}>${money(r.opera)}</td>
            <td className={`${tdN} ${negColor(r.integrity)}`}>${money(r.integrity)}</td>
            <td className={`${tdN} ${r.diferencia && Math.abs(r.diferencia) >= 0.01 ? "text-amber-400" : negColor(r.diferencia) || "text-white/50"}`}>${money(r.diferencia)}</td>
            <td className={`px-3 py-1.5 font-medium ${ESTADO_STYLE[r.estado] || "text-white"}`}>{ESTADO_LABEL[r.estado] || r.estado}</td>
          </tr>
        ))}
      </tbody>
    </table>
  ...
  return (
    ...
    {source && (
      <div className="text-xs text-white/60">
        Source:{" "}
        <span className={`rounded px-2 py-0.5 font-medium ${official ? "bg-emerald-500/15 text-emerald-300" : "bg-amber-500/15 text-amber-300"}`}>
          {source}
        </span>
      </div>
    )}
      ...
      <tbody>
        {groups.map((g) => {
          ...
          return (
            <Fragment key={g.type}>
              {g.rows.map((r) => (
                <tr key={r.tcode} className="border-t border-white/5">
                  <td className="px-3 py-1.5 font-mono text-white/80">{r.tcode}</td>
                  <td className={td}>{r.description}</td>
                  <td className="px-3 py-1.5 text-white/50">{r.type}</td>
              ...
              <tr className="border-t border-white/10 bg-[#1E2130]/60">
                <td className={`${td} font-medium text-white/60`} colSpan={3}>Subtotal {g.type}</td>
        ...
        <tr className="border-t-2 border-white/20 bg-[#1E2130]">
          <td className={`${td} font-bold`} colSpan={3}>GRAND TOTAL</td>
          <td className={`${tdN} font-bold ${negColor(grandTotal.total) || "text-emerald-400"}`}>${money(grandTotal.total)}</td>
...
function Ledgers({ ledgers, day, onSaved }: { ledgers: Ledger[]; day: string; onSaved: () => void }) {
  ...
  return (
    <div className="space-y-2">
      <p className="text-xs text-white/50">
        Running balance: <b className="text-white/70">Closing = Opening + Day&apos;s Movement</b>. The
      ...
      <table className="w-full rounded-lg border border-white/10 text-xs">
        ...
        <tbody>
          {ledgers.map((l) => (
            <tr key={l.ledger} className="border-t border-white/5">
                ...
                {edit === l.ledger ? (
                  <span className="flex items-center justify-end gap-1">
                    ...
                    <input value={amount} onChange={(e) => setAmount(e.target.value)} type="number"
                      autoFocus
                      className="w-28 rounded border border-accent bg-[#0F1118] px-2 py-1 text-right text-white" />
                  </span>
                ) : `$${money(l.opening)}`}
              </td>
              <td className={`${tdN} ${negColor(l.movement)}`}>${money(l.movement)}</td>
              <td className={`${tdN} font-medium ${negColor(l.closing) || "text-white"}`}>${money(l.closing)}</td>
              <td className="px-3 py-1.5 text-white/50">
                {l.source === "Trial Balance"
                  ? <span className="rounded bg-emerald-500/15 px-1.5 py-0.5 text-[11px] text-emerald-300">Trial Balance</span>
                  : l.anchored ? <span className="text-sky-400">anchor {l.anchor_date}</span> : "carried over"}
                ...
                {edit === l.ledger ? (
                  <span className="flex items-center gap-1">
                    <input value={note} onChange={(e) => setNote(e.target.value)} placeholder="note — optional, NOT the balance"
                      ...
                      className="w-40 rounded border border-white/15 bg-[#0F1118] px-2 py-1 text-white/70 placeholder:text-white/30 placeholder:italic" />
                    ...
                    <button onClick={() => setEdit(null)} className="rounded bg-white/10 px-2 py-1 text-[11px] text-white/70">✕</button>
                  </span>
                ) : (
                  <span className="flex gap-1">
                    <button onClick={() => toggleDetail(l.ledger)}
                      className={`rounded px-2 py-1 text-[11px] ${openDetail === l.ledger ? "bg-accent text-white" : "bg-white/10 text-white/70 hover:text-white"}`}>
                      {openDetail === l.ledger ? "▲ folios" : "▼ folios"}
                    </button>
                    <button onClick={() => startEdit(l)} className="rounded bg-white/10 px-2 py-1 text-[11px] text-white/70 hover:text-white">Edit opening</button>
                  </span>
                )}
              </td>
            </tr>
      ...
      {openDetail && (
        <div className="rounded-lg border border-white/10 bg-[#1E2130]/40 p-3 text-xs">
          <div className="mb-2 font-medium text-white/80">
          ...
          {!detail ? (
            ...
            <div className="space-y-3">
              <div>
                <div className="mb-1 text-[11px] uppercase tracking-wide text-white/50">Day&apos;s movement by TCode (ties to the movement)</div>
                {detail.movement_detail.length === 0 ? (
                  <div className="text-white/50">No movement this day.</div>
                ) : (
                  <table className="w-full">
                    <thead><tr>{["TCode", "Description", "Type", "Amount"].map((h) => <th key={h} className={th}>{h}</th>)}</tr></thead>
                    <tbody>
                      {detail.movement_detail.map((m) => (
                        <tr key={m.tcode} className="border-t border-white/5">
                          <td className="px-3 py-1 font-mono text-white/80">{m.tcode}</td>
                          <td className={td}>{m.description}</td>
                          <td className="px-3 py-1 text-white/50">{m.type}</td>
                          <td className={`${tdN} ${negColor(m.amount)}`}>${money(m.amount)}</td>
                        </tr>
                      ))}
                      <tr className="border-t border-white/20 bg-[#1E2130]">
              ...
              {detail.ledger === "ar" && detail.folios.length > 0 && (
                <div>
                  <div className="mb-1 flex items-center gap-2 text-[11px] uppercase tracking-wide text-white/50">
                    City Ledger invoices (companies / agencies)
                    {detail.ties_to_movement
                      ? <span className="rounded bg-emerald-500/15 px-1.5 text-[10px] text-emerald-300 normal-case">✓ ties to movement</span>
                      : <span className="rounded bg-red-500/15 px-1.5 text-[10px] text-red-300 normal-case">⚠ does not tie</span>}
                  </div>
                  <table className="w-full">
                    ...
                    <tbody>
                      {(detail.folios as ArInvoice[]).map((f) => (
                        <tr key={f.invoice_no} className="border-t border-white/5">
                          <td className="px-3 py-1.5 font-mono text-white/80">{f.invoice_no}</td>
                          <td className={td}>{f.account_name}{f.account_number ? <span className="text-white/40"> · {f.account_number}</span> : null}</td>
                          <td className={td}>{f.guest_name}</td>
                          <td className="px-3 py-1.5 text-white/40">{f.arrival_date} → {f.departure_date}</td>
              ...
              {detail.ledger === "guest" && detail.folios.length > 0 && (
                <div>
                  <div className="mb-1 text-[11px] uppercase tracking-wide text-white/50">Folios (day&apos;s settled documents)</div>
              <table className="w-full">
                ...
                <tbody>
                  {(detail.folios as Folio[]).map((f) => (
                    <Fragment key={f.bill_no}>
                      <tr className="border-t border-white/5 cursor-pointer hover:bg-white/5"
                        onClick={() => setOpenFolio(openFolio === f.bill_no ? null : f.bill_no)}>
                        <td className="px-3 py-1.5 font-mono text-white/80">{openFolio === f.bill_no ? "▾ " : "▸ "}{f.bill_no}</td>
                        <td className={td}>{f.guest_name}</td>
                        <td className="px-3 py-1.5 text-emerald-400">{f.status}</td>
                        <td className={`${tdN} font-medium ${negColor(f.total_amount)}`}>${money(f.total_amount)}</td>
                        <td className="px-3 py-1.5 text-white/40">{f.lines.length} lines</td>
                      </tr>
                      {openFolio === f.bill_no && f.lines.map((ln, i) => (
                        <tr key={`${f.bill_no}-${i}`} className="bg-[#0F1118]/60 text-white/60">
              ...
              <p className="text-[11px] text-white/40">{detail.note}</p>
            </div>
          )}
        </div>
      )}
  ...
  return (
    <div>
      ...
      <table className="w-full rounded-lg border border-white/10 text-xs">
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r[keyName]} className="border-t border-white/5">
              <td className="px-3 py-1.5 text-white/80">
                {r.name || r[keyName]}
                {r.name && <span className="ml-1 font-mono text-[10px] text-white/40">({r[keyName]})</span>}
              </td>
              <td className={`${tdN} ${negColor(r.rooms)}`}>{int(r.rooms)}</td>
              <td className={`${tdN} ${negColor(r.persons)}`}>{int(r.persons)}</td>
              <td className={`${tdN} ${r.noshow_rooms ? "text-amber-400" : negColor(r.noshow_rooms)}`}>{int(r.noshow_rooms)}</td>
              <td className={`${tdN} ${r.cancel_rooms ? "text-amber-400" : negColor(r.cancel_rooms)}`}>{int(r.cancel_rooms)}</td>
            </tr>
          ))}
          <tr className="border-t border-white/20 bg-[#1E2130]">
...
function Estadisticas({ o }: { o: Occupancy }) {
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-4 gap-3">
        <Kpi label="Occupied Rooms" value={int(o.totals.rooms)} tone="text-emerald-400" />
        <Kpi label="Persons (PAX)" value={int(o.totals.persons)} tone="text-sky-400" />
        <Kpi label="No-show" value={int(o.totals.noshow_rooms)} tone={o.totals.noshow_rooms ? "text-amber-400" : "text-white/60"} />
        <Kpi label="Canceled" value={int(o.totals.cancel_rooms)} tone={o.totals.cancel_rooms ? "text-amber-400" : "text-white/60"} />
      </div>
      ...
      <div>
        <div className="mb-1 text-[11px] uppercase tracking-wide text-white/50">Detail (market / class / type)</div>
        <table className="w-full rounded-lg border border-white/10 text-xs">
          <thead className="bg-[#1E2130]"><tr>
            ...
            {o.records.map((r, i) => (
              <tr key={i} className="border-t border-white/5">
                <td className="px-3 py-1.5 text-white/80">
                  ...
                  {r.market_name && <span className="ml-1 font-mono text-[10px] text-white/40">({r.market_code})</span>}
                </td>
                <td className="px-3 py-1.5 text-white/70">
                  {r.room_class_name || r.room_class}
                  {r.room_class_name && <span className="ml-1 font-mono text-[10px] text-white/40">({r.room_class})</span>}
                </td>
                <td className="px-3 py-1.5 font-mono text-white/70">{r.room_type}</td>
                <td className={`${tdN} ${negColor(r.rooms)}`}>{int(r.rooms)}</td>
                <td className={`${tdN} ${negColor(r.persons)}`}>{int(r.persons)}</td>
...
function InhouseComp({ o }: { o: Occupancy }) {
  ...
  return (
    <div>
      <div className="mb-1 text-[11px] uppercase tracking-wide text-white/50">In-House &amp; Complimentary Rooms (COM / INHOUSE)</div>
      {rows.length === 0 ? (
        <div className="rounded-lg border border-white/10 bg-[#171A28] px-3 py-2 text-xs text-white/50">
            ...
            {rows.map((r, i) => (
              <tr key={i} className="border-t border-white/5">
                ...
                <td className="px-3 py-1.5 font-mono text-white/70">
                  {r.room_type || "—"}
                  {r.room_class_name && <span className="ml-1 font-sans text-[10px] text-white/40">{r.room_class_name}</span>}
                </td>
...
function PosCheckRow({ r }: { r: PosCheckCheck }) {
  ...
  return (
    <tr className="border-t border-white/5">
      ...
      <td className={`${tdN} ${ok ? negColor(r.diferencia) || "text-white/50" : "text-amber-400 font-medium"}`}>${money(r.diferencia)}</td>
      <td className={`px-3 py-1.5 font-medium ${ESTADO_STYLE[r.estado] || "text-white"}`}>{ok ? "✓ Reconciled" : "⚠ " + (ESTADO_LABEL[r.estado] || r.estado)}</td>
    </tr>
  );
}
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-5 gap-3">
        <Kpi label="Net Sales" value={`$${money(s.ventas_netas)}`} tone="text-emerald-400" />
        <Kpi label="Service Charges" value={`$${money(s.cargos_servicio)}`} tone="text-sky-400" />
        <Kpi label="Total Sales" value={`$${money(s.total_ventas)}`} tone="text-white" />
        <Kpi label="Voids" value={`$${money(s.voids)}`} tone={s.voids ? "text-amber-400" : "text-white/60"} />
        <Kpi label="Room Charges → Opera" value={`$${money(s.room_charge_confirmado)}`} tone="text-emerald-400" />
      </div>
            ...
            <tbody>
              {pos.room_charges.map((c, i) => (
                <tr key={i} className="border-t border-white/5">
                  ...
                  <td className="px-3 py-1.5 font-mono text-white/80">{c.check_num}</td>
                  <td className="px-3 py-1.5 text-white/50">{c.hora}</td>
                  <td className={`${tdN} ${negColor(c.monto)}`}>${money(c.monto)}</td>
                  <td className="px-3 py-1.5 font-medium text-emerald-400">✓ AnswerStat=OK</td>
                </tr>
              ))}
              <tr className="border-t border-white/20 bg-[#1E2130] font-bold">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <div className="mb-1 text-[11px] uppercase tracking-wide text-white/50">By Payment Method</div>
          <table className="w-full rounded-lg border border-white/10 text-xs">
            <thead className="bg-[#1E2130]"><tr>
            ...
            <tbody>
              {pos.by_payment.map((p) => (
                <tr key={p.forma} className="border-t border-white/5">
                  <td className={`${td} ${p.forma.toUpperCase().includes("ROOM CHARGE") ? "text-emerald-400 font-medium" : ""}`}>{p.forma}</td>
        ...
        <div>
          <div className="mb-1 text-[11px] uppercase tracking-wide text-white/50">By Employee (cashier control)</div>
          <table className="w-full rounded-lg border border-white/10 text-xs">
            <thead className="bg-[#1E2130]"><tr>
            ...
            <tbody>
              {pos.by_employee.map((e) => (
                <tr key={e.empleado} className="border-t border-white/5">
...
function OtbVsRevenue({ otb }: { otb: OtbData }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        <div className="mb-1 text-[11px] uppercase tracking-wide text-white/50">Detail of each OTB report</div>
        <table className="w-full rounded-lg border border-white/10 text-xs">
          <thead className="bg-[#1E2130]"><tr>
          ...
          <tbody>
            {scopes.filter((s) => s.d).map(({ key, label, d }) => (
              <tr key={key} className="border-t border-white/5">
                ...
                <td className="px-3 py-1.5 text-white/40">{d!.source_file}</td>
              </tr>
            ))}
            {otb.actual_stats && (
              <tr className="border-t border-white/20 bg-[#1E2130]">
                <td className={`${td} font-bold text-sky-400`}>Actual</td>
                <td className={`${tdN} font-bold ${negColor(otb.rooms_only_recon?.actual_integrity) || "text-sky-400"}`}>${money(otb.rooms_only_recon?.actual_integrity ?? null)}</td>
                <td className={`${tdN} font-bold ${negColor(otb.actual_stats.rn) || "text-sky-400"}`}>{int(otb.actual_stats.rn)}</td>
                <td className={`${tdN} font-bold ${negColor(otb.actual_stats.pax) || "text-sky-400"}`}>{int(otb.actual_stats.pax)}</td>
                <td className={`${tdN} font-bold ${negColor(otb.actual_stats.available) || "text-sky-400"}`}>
                ...
                <td className={`${tdN} font-bold ${negColor(otb.actual_stats.adr) || "text-sky-400"}`}>${money(otb.actual_stats.adr)}</td>
                <td className={`${tdN} font-bold text-sky-400`}>{(otb.actual_stats.occupancy_pct * 100).toFixed(1)}%</td>
                <td className="px-3 py-1.5 text-white/40">Revenue/ADR: Integrity · RN/Pax/Inv: fact_room_stat · Available: fixed at 30</td>
              </tr>
            )}
          </tbody>
        </table>
        {otb.non_room_revenue !== null && (
          <p className="mt-1 text-[11px] text-white/40">
            For reference: Full Revenue − Rooms Only = <span className={`text-white/70 ${negColor(otb.non_room_revenue)}`}>${money(otb.non_room_revenue)}</span> of
              ...
              {([
                ...
                return (
                  <tr key={label} className="border-t border-white/5">
                    ...
                    <td className={`${tdN} ${ok ? negColor(dif) || "text-white/50" : "text-amber-400 font-medium"}`}>{ok ? "0" : (dif > 0 ? "+" : "") + fmt(dif)}</td>
                    <td className={`px-3 py-1.5 font-medium ${ok ? "text-emerald-400" : "text-amber-400"}`}>{ok ? "✓ Reconciled" : "⚠ Review"}</td>
                  </tr>
                );
              })}
  ...
  return (
    <div className="space-y-2">
      ...
      {findings.length === 0 && !loading ? (
        <div className="rounded-lg border border-white/10 bg-[#1E2130] p-6 text-sm text-emerald-400">
        ...
        <table className="w-full rounded-lg border border-white/10 text-xs">
          ...
          <tbody>
            {findings.map((f) => (
              <Fragment key={f.id}>
                <tr className="border-t border-white/5">
                  <td className="px-3 py-1.5 font-mono text-white/60">{f.business_date}</td>
                  <td className="px-3 py-1.5 font-mono text-white/80">{f.tcode ?? "—"}</td>
                  <td className={td}>{f.area}</td>
                  <td className="px-3 py-1.5 text-amber-400">{(f.tipo_desviacion && ESTADO_LABEL[f.tipo_desviacion]) || f.tipo_desviacion}</td>
                  <td className={`${tdN} ${negColor(f.monto)}`}>${money(f.monto)}</td>
                  <td className="px-3 py-1.5 text-white/50">{f.comentario}</td>
                  <td className="px-3 py-1.5 text-amber-400">{f.estado === "abierto" ? "open" : f.estado === "cerrado" ? "closed" : f.estado}</td>
                  <td className="px-3 py-1.5">
                    {editingId !== f.id && (
                      <button onClick={() => startEdit(f)}
                        className="rounded bg-white/10 px-2 py-1 text-[11px] text-white/70 hover:text-white">
                        Change status
                      </button>
                    )}
                  </td>
                ...
                {editingId === f.id && (
                  <tr className="border-t border-white/5 bg-[#1E2130]/50">
                    <td colSpan={8} className="px-3 py-3">
                      <div className="flex flex-col gap-2">
                        <div className="flex items-center gap-2">
                          <label className="text-[11px] text-white/60">New status:</label>
                          <select value={draftEstado} onChange={(e) => setDraftEstado(e.target.value)}
                            className="rounded border border-white/15 bg-[#0F1118] px-2 py-1 text-xs text-white">
                        ...
                        <textarea value={draftNote} onChange={(e) => setDraftNote(e.target.value)}
                          ...
                          rows={4}
                          className="w-full rounded border border-white/15 bg-[#0F1118] px-2 py-1.5 text-xs text-white placeholder:text-white/30" />
                        ...
                        <div className="flex gap-2">
                          ...
                          <button onClick={() => setEditingId(null)}
                            className="rounded bg-white/10 px-3 py-1 text-[11px] text-white/70">
                            Cancel
                          </button>
                        </div>
                      </div>
...
function Hallazgos({ findings }: { findings: Finding[] }) {
  if (findings.length === 0)
    return <div className="rounded-lg border border-white/10 bg-[#1E2130] p-6 text-sm text-emerald-400">No open findings.</div>;
  return (
    <table className="w-full rounded-lg border border-white/10 text-xs">
      <thead className="bg-[#1E2130]"><tr>
      ...
      <tbody>
        {findings.map((f, i) => (
          <tr key={`${f.tcode ?? "sin-tcode"}-${f.area ?? ""}-${i}`} className="border-t border-white/5">
            <td className="px-3 py-1.5 font-mono text-white/60">{f.business_date ?? "—"}</td>
            <td className="px-3 py-1.5 font-mono text-white/80">{f.tcode ?? "—"}</td>
            <td className={td}>{f.area}</td>
            <td className="px-3 py-1.5 text-amber-400">{(f.tipo_desviacion && ESTADO_LABEL[f.tipo_desviacion]) || f.tipo_desviacion}</td>
            <td className={`${tdN} ${negColor(f.monto)}`}>${money(f.monto)}</td>
            <td className="px-3 py-1.5 text-white/60">{f.estado === "abierto" ? "open" : f.estado === "cerrado" ? "closed" : f.estado}</td>
            <td className="px-3 py-1.5 text-white/50">{f.comentario}</td>
          </tr>
        ))}
      </tbody>
